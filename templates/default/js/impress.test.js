...
( function( document, window ) {
	...
	// `altPrev` API function goes to previous non-linear step
	var altPrev = function() {
	    var altPrev = activeStep.data('altPrev');
	    if( altPrev ) {  
	    } else {
            	altPrev = steps.indexOf( activeStep ) - 1;
            	altPrev = altPrev >= 0 ? steps[ altPrev ] : steps[ steps.length - 1 ];
	    }
            return goto( altPrev );
	};

	// `altNext` API function goes to next non-linear step
	var altNext = function() {
	    var altNext = activeStep.data('altNext');
	    if( altNext ) {  
	    } else {
            	altNext = steps.indexOf( activeStep ) - 1;
            	altNext = altNext >= 0 ? steps[ altNext ] : steps[ steps.length - 1 ];
	    }
            return goto( altNext );
	};
